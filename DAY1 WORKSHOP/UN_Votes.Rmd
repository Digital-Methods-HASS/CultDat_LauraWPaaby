---
title: "First CulturalDataScience"
author: "LAURA PAAABY"
date: '2022-08-30'
...
```{r, message = FALSE}
pacman::p_load(unvotes, tidyverse, lubridate, DT)
```
...
### trying to get DT package to work hmmm 
remove.packages("DT")
...
```{r country-list}
country_list <- c("United States", "Turkey")
```
...
### visualisiing America vs. DK
``` {r}
new_countries <- c("Denmark", "United States", "Poland")
...
```
### selecting all countries starting with D
``` {r}
d_countries <- un_votes %>%
  filter(country %in% grep("^D", country, value = TRUE))
...
d_countries %>%
  ...
  ggplot(mapping = aes(x = year, y = percent_yes, color = country)) +
    geom_line() +
    labs(
      title = "Percentage of 'Yes' votes in the UN General Assembly by Countries Starting w. D",
      subtitle = "1946 to 2015",
      y = "% Yes",
      x = "Year",
      color = "Country"
